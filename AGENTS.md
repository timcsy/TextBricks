...
│   └── 資料夾結構儲存
├── 🗂️ 統一導航系統
│   ├── NavigationItem (folder/template/link)
│   ├── 階層式主題 (topic.json)
│   └── 跨主題連結機制
...
- 📁 **平台無關儲存** - 不再綁定 vscode/src/data
- 🗂️ **統一導航項目** - folder/template/link 三種類型
- 🛠️ **全新 Manager** - 完整的圖形化管理界面
...
- [ ] **統一導航項目系統**
  - folder (📁) - 子主題，右側 > 按鈕進入
  - template (📄) - 程式碼模板，保持現有操作
  - link (🔗) - 跨主題連結，繼承目標操作
- [ ] **topic.json 檔案格式**
  - 主題基本資訊和階層關係
  - 可自訂 templates/links 資料夾名稱
  - inline/file/url 三種文檔類型支援
- [ ] **階層導航邏輯**
  - 麵包屑路徑顯示
  - 上下層級導航
  - 跨主題連結跳轉
...
### ⚡ 立即執行 (v0.3.0)
1. **階層式主題系統重構** - 統一導航項目、平台無關儲存
2. **TextBricks Manager v3** - 全新的管理界面和 Import/Export
3. **資料遷移系統** - 從舊格式無痛升級到新架構
4. **平台無關儲存** - 系統標準目錄和 Scope 管理
...
- ✅ 模板插入縮排問題修復
- 🎯 **v0.3.0 階層式主題系統重構**
  - 統一導航項目系統 (folder/template/link)
  - 平台無關儲存架構
  - TextBricks Manager v3 開發
  - 資料遷移和向後相容系統
...
### 2025-09-24 - v0.3.0 階層式主題系統完整規劃完成
- **執行者**：Claude Code
- **變更**：
  - 🎯 **重大架構規劃**：完成 v0.3.0 階層式主題系統重構的完整設計
  - 🗂️ **統一導航項目系統**：設計 folder/template/link 三種類型的統一操作邏輯
  - 📁 **平台無關儲存架構**：從 packages/vscode/src/data 遷移到系統標準目錄
  - 🛠️ **TextBricks Manager v3**：全新的樹狀導航管理界面設計
  - 📦 **Import/Export 策略**：ZIP + JSON 雙格式靈活匯入匯出
  - 🔄 **資料遷移系統**：確保從舊格式無痛升級的完整方案
- **技術創新**：
  - 📋 **topic.json 格式**：每個主題獨立配置，支援自訂資料夾名稱
  - 🗂️ **Scope 管理**：local + 用戶自創 scope 的靈活組織方式
  - 🔗 **跨主題連結**：支援 folder 和 template 兩種目標的 link 機制
  - 📖 **統一文檔系統**：inline/file/url 三種類型，topic 和 template 都支援
- **文件更新**：
  - ✅ **AGENTS.md 重構**：更新架構狀態、發展規劃、當前優先級
  - ✅ **規劃文件創建**：V0.3.0_ARCHITECTURE_PLAN.md, HIERARCHICAL_TOPICS_DESIGN.md 等
  - ✅ **時程表調整**：反映 v0.3.0 為當前最高優先級的實際開發計劃
- **戰略意義**：
  - 🚀 **徹底平台無關**：為 Vim、Sublime Text 等多平台擴展做好準備
  - 🎯 **用戶體驗革新**：檔案系統般直觀的階層導航體驗
  - 🔧 **開發者友善**：完整的管理工具和批次操作支援
- **實施階段**：6個階段，預估 4-6 週完成
- **狀態**：v0.3.0 完整規劃完成，進入實施準備階段
- **下一步**：開始實施 Phase 1 資料結構重構
